        ...
        if ($('#placa').val() != '') {
            if ($('#tipo').val() > 0) {
                if ($('#anio').val() != '') {
                    ...
                    var correo = '';
                    var placa = $('#placa').val();
                    ultimodigito = placa.substr(placa.length - 1);

                    if ($('#anio').val() <= 2010) {
                        respuesta = '<h4 style="color: red;">Búsqueda nó válida, el año de fabricación debe ser del 2011 hacia adelante.</h4>';
                        $('#resultado').html(respuesta);
                        $("html, body").animate({scrollTop: $("#titulobuscador").offset().top}, "slow");
                        $('#anio').focus();
                    } else {
                        if ($.isNumeric(ultimodigito)) {
//                            console.log('es número');
                            digitoplaca = Number(ultimodigito);
                            var correo = $("#correo").val();
                            var tipo = Number($('#tipo').val());
                            var anio = Number($('#anio').val());

                            switch (digitoplaca) {
                                case 0:
                                    mesplaca = 'Enero y Febrero';
                                    break;
                                case 1:
                                    mesplaca = 'Marzo';
                                    break;
                            ...
                            respuesta = '<h3>RESULTADO</h3>';
                            respuesta = respuesta + '<h4>Fecha límite: <span style=font-size:25px>' + mesplaca + ' ' + anio + '</span></h4>';
                            $("html, body").animate({scrollTop: $("#titulobuscador").offset().top}, "slow");

                            var correoGet = $("#correo").val();
                            var placaGet = $("#placa").val();
                            var tipoGet = $("#tipo").val();
                            var anioGet = $("#anio").val();

                            $.ajax({
                                url: 'php/grabar-consulta-cronograma.php?correo=' + correoGet + '&placa=' + placaGet + '&tipo=' + tipoGet + '&anio=' + anioGet + '&resultado=' + mesplaca + ' ' + anio,
                                type: 'POST',
//                                data: fdata,
                                cache: false,
                                contentType: false,
...
//                        $("#submit_pedidos button").prop('disabled', 'disabled');
                                },
...
//                    
                        ...
                        $('#resultado').html(respuesta);
//                        $("#resultado").animate({scrollTop: 0}, "slow");
                    }
                } else {
                    console.log('Ingrese el Año de fabricación');
                    $('#anio').focus();
                }
            } else {
                console.log('Seleccione el Tipo de vehículo');
                $('#tipo').focus();
            }
        } else {
            console.log('Ingrese el Número de placa');
            $('#placa').focus();
        }
    });
    ...
    $('.mailchimp').ajaxChimp({
        url: "http://Revisiones Técnicas del Perú | RTP.us5.list-manage.com/subscribe/post?u=91b69df995c1c90e1de2f6497&amp;id=aa0f2ab5fa", //Replace with your own mailchimp Campaigns URL.
